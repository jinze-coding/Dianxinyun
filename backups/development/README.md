...
该目录保存 2026-07-28 导出的本地单项目演示环境快照，用于恢复当前联调数据。数据均为虚构演示信息，不应直接用于生产环境。
...
数据库中的 `file_resource` 记录与该目录中的 4 个文件相互对应：1 份工程资料、2 张巡检图片和 1 张质量问题图片。若使用 MinIO，需要按部署环境上传文件并调整存储配置。
