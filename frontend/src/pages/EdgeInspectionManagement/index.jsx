...
export default function EdgeInspectionManagement({ projectId, theme: T, currentUser, businessTarget }) {
  ...
  const [activeView, setActiveView] = useNavigationTab('edgeTab', 'todo', ['todo', 'records', 'settings']);
  ...
  useEffect(() => {
    currentEdgeViewRef.current = viewProjectRef.current === projectId ? activeView : 'todo';
    ...
    if (viewProjectRef.current !== projectId) setActiveView('todo');
    viewProjectRef.current = projectId;
    setPointTypes([]); setPoints([]); setUsers([]); setSetting(null); setSettingEditor(null);
