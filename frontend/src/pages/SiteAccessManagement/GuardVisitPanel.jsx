...
export default function GuardVisitPanel({ projectId, canManage, canExport, onOpenProfiles }) {
  ...
  const saveCorrection = async () => {
    ...
    if (!form.visitorCompany.trim() || !form.contactName.trim() || !/^1[3-9]\d{9}$/.test(form.contactPhone.trim())) {
      return setError('请完整填写单位、主联系人和正确手机号');
    }
    if (invalidPhone >= 0) return setError(`第 ${invalidPhone + 1} 位同行人员手机号格式不正确`);
  ...
  return <div className="guard-visit-panel">
    ...
    <section className="site-access-filter-card guard-filter-card">
      ...
      <div className="site-access-keyword"><input value={keywordInput} onChange={(event) => setKeywordInput(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && setKeyword(keywordInput.trim())} placeholder="登记编号、单位、联系人、车牌" /><button type="button" onClick={() => setKeyword(keywordInput.trim())}>查询</button></div>
    ...
    <section className="site-access-table-card">
      <div className="site-access-table-wrap"><table><thead><tr><th>登记编号</th><th>单位 / 联系人</th><th>人数</th><th>车辆</th><th>登记时间</th><th>有效截止</th><th>状态</th><th>操作</th></tr></thead><tbody>
    ...
    {editing && <GuardModal title={`纠错 · ${editing.registrationNo}`} onClose={() => setEditing(null)}>
      <div className="site-access-form-grid">
        <label className="site-access-field full"><span>外访单位 *</span><input value={form.visitorCompany} onChange={(event) => setForm({ ...form, visitorCompany: event.target.value })} /></label>
        <label className="site-access-field"><span>主联系人 *</span><input value={form.contactName} onChange={(event) => setForm({ ...form, contactName: event.target.value })} /></label><label className="site-access-field"><span>手机号 *</span><input maxLength="11" value={form.contactPhone} onChange={(event) => setForm({ ...form, contactPhone: event.target.value })} /></label>
        <div className="site-access-companions full"><div className="site-access-companion-head"><strong>同行人员（单位、姓名、手机号均选填）</strong><button type="button" disabled={form.companions.length >= 49} onClick={() => setForm({ ...form, companions: [...form.companions, { personCompany: '', personName: '', personPhone: '' }] })}>添加同行人</button></div>{form.companions.map((person, index) => <div className="site-access-companion-row" key={index}><input placeholder="单位（选填）" value={person.personCompany} onChange={(event) => updateCompanion(index, 'personCompany', event.target.value)} /><input placeholder="姓名（选填）" value={person.personName} onChange={(event) => updateCompanion(index, 'personName', event.target.value)} /><input placeholder="手机号（选填）" maxLength="11" value={person.personPhone} onChange={(event) => updateCompanion(index, 'personPhone', event.target.value)} /><button className="danger" type="button" onClick={() => setForm({ ...form, companions: form.companions.filter((_, itemIndex) => itemIndex !== index) })}>移除</button></div>)}</div>
    ...
    {detail && <div className="site-access-drawer-mask" onMouseDown={() => setDetail(null)}><aside className="site-access-drawer" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="site-access-detail-grid">{[['项目', detail.projectName], ['外访单位', detail.visitorCompany], ['联系人', `${detail.contactName} ${detail.contactPhone || ''}`], ['出行方式', detail.travelMode === 'DRIVING' ? `驾车 · ${detail.vehiclePlate || '-'}` : '非驾车'], ['登记时间', formatDateTime(detail.registeredTime)], ['有效截止', formatDateTime(detail.validUntil)], ['资料来源', detail.sourceProfileName || '本次手工填写'], ['外访备注', detail.visitorRemark || '-']].map(([label, value]) => <div key={label}><span>{label}</span><b>{value}</b></div>)}</div>
      <h3>登记人员（{detail.visitors?.length || 0}）</h3><div className="site-access-person-list">{(detail.visitors || []).map((person, index) => <div key={index}><span>{person.personType === 'CONTACT' ? '主联系人' : '同行人员'}</span><div><b>单位：{person.personCompany || '-'}</b><b>姓名：{person.personName || '-'}</b><b>手机号：{person.personPhone || '-'}</b></div></div>)}</div>
