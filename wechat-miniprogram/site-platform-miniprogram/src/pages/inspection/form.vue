...
function addPhoto(type: 'outer' | 'inner') {
  ...
  uni.chooseImage({ count: PHOTO_MAX - target.value.length, success: (result) => { target.value = [...target.value, ...result.tempFilePaths].slice(0, PHOTO_MAX); } });
}
...
async function confirmSubmit() {
  ...
  try {
    const outerPhotoFileIds = await uploadPhotoIds(outerPhotos.value, 'INSPECTION_OUTER_PHOTO', { projectId: box.value.projectId, businessType: 'inspection_record' });
    uploadedFileIds.push(...outerPhotoFileIds);
    const innerPhotoFileIds = await uploadPhotoIds(innerPhotos.value, 'INSPECTION_INNER_PHOTO', { projectId: box.value.projectId, businessType: 'inspection_record' });
  ...
  } catch (error) { await deleteFileResources(uploadedFileIds); showToast(error instanceof Error ? error.message : '提交失败'); }
...
<template>
  <view class="flow-page inspection-page">
    ...
    <scroll-view class="flow-scroll" scroll-y enable-flex :style="scrollStyle">
      <view v-if="box" class="flow-content inspection-content">
...
.abnormal-input { box-sizing: border-box; width: 100%; height: 98rpx; margin-top: 12rpx; padding: 15rpx; border: 1rpx solid #f0cfca; border-radius: 13rpx; background: #fff8f7; color: #3b4657; font-size: 21rpx; line-height: 1.45; }
