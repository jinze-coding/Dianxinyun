  ...
  <view class="checkin-shell">
    ...
    <view v-else-if="meeting" class="checkin-content">
      ...
      <template v-if="!reservedMode">
        ...
        <view class="checkin-card form-card">
          <view class="section-head"><view><text>同行到场人员</text><text>每位实际到场人员都必须填写姓名</text></view><button @click="addCompanion">添加</button></view>
          <text v-if="!companions.length" class="field-hint">无同行人员可跳过，本人无需重复添加。</text>
          <view v-for="(person, index) in companions" :key="index" class="companion-card">
            <view class="companion-head"><text>同行人员 {{ index + 1 }}</text><button @click="removeCompanion(index)">移除</button></view>
            <label class="checkin-field"><text>单位（选填）</text><input v-model="person.personCompany" maxlength="200" placeholder="请输入同行人员单位" placeholder-class="checkin-placeholder" :cursor-spacing="24" /></label>
            <label class="checkin-field"><text>姓名 *</text><input v-model="person.personName" maxlength="50" placeholder="请输入同行人员姓名" placeholder-class="checkin-placeholder" :cursor-spacing="24" /></label>
            <label class="checkin-field"><text>手机号码（选填）</text><input v-model="person.personPhone" type="number" maxlength="11" placeholder="请输入同行人员手机号码" placeholder-class="checkin-placeholder" :cursor-spacing="24" /></label>
          </view>
        </view>
        <view class="checkin-card form-card">
          <text class="block-title">出行信息</text>
          <view class="travel-tabs"><button :class="{ active: travelMode === 'OTHER' }" @click="travelMode = 'OTHER'">非驾车</button><button :class="{ active: travelMode === 'DRIVING' }" @click="travelMode = 'DRIVING'">驾车</button></view>
...
.state-card.error text:first-child{color:#a63f3f;font-size:32rpx;font-weight:850}
.state-card button{min-height:68rpx;padding:0 32rpx;border-radius:14rpx;background:#1c628f;color:#fff;font-size:23rpx}
.meeting-card{background:linear-gradient(145deg,#163853,#256b95);color:#fff}
.eyebrow,.meeting-title{display:block}
.eyebrow{font-size:20rpx;opacity:.76}
.meeting-title{margin:14rpx 0 26rpx;font-size:34rpx;line-height:1.45;font-weight:850;overflow-wrap:anywhere}
.meeting-grid{display:grid;grid-template-columns:128rpx minmax(0,1fr);gap:14rpx 16rpx;font-size:23rpx;line-height:1.6}
.meeting-grid text:nth-child(odd){opacity:.65}
.location-notice{display:flex;gap:14rpx;border-radius:16rpx;padding:18rpx 22rpx;background:#fff7e7;color:#795b24;font-size:21rpx;line-height:1.55}
.location-notice text:first-child{flex:none;font-weight:800}
.error-banner{border:1rpx solid #eabdb9;border-radius:14rpx;padding:18rpx;background:#fff1ef;color:#a33d37;font-size:22rpx}
.section-head{display:flex;align-items:center;justify-content:space-between;gap:16rpx}
.section-head>view{display:flex;min-width:0;flex:1;flex-direction:column;gap:8rpx}
.section-head>view text:first-child,.block-title{display:block;font-size:28rpx;line-height:1.5;font-weight:800}
.section-head>view text:last-child{color:#7b8999;font-size:22rpx;line-height:1.6;overflow-wrap:anywhere}
.section-head>text{color:#25704f;font-size:22rpx;font-weight:750}
.attendee-row{display:flex;align-items:center;gap:14rpx;margin-top:18rpx;border:1rpx solid #dce5ed;border-radius:16rpx;padding:18rpx;background:#fafcfe}
.attendee-row.checked{background:#f0faf4;border-color:#b8dfc8}
.attendee-row>view{display:flex;min-width:0;flex:1;flex-direction:column;gap:6rpx}
.attendee-row>view>text:first-child{font-size:25rpx;font-weight:800}
.attendee-row>view>text:nth-child(2){color:#718091;font-size:20rpx}
.attendee-row input{box-sizing:border-box;width:100%;height:80rpx;min-height:40px;margin-top:6rpx;border:1rpx solid #d5e0e7;border-radius:12rpx;padding:0 18rpx;background:#fff;font-size:26rpx;line-height:normal}
.attendee-status{color:#277655;font-size:20rpx}
...
.travel-tabs button.active{border-color:#176b9f;background:#176b9f;color:#fff}
.privacy-card label{display:flex;align-items:flex-start;gap:12rpx;font-size:24rpx;font-weight:700;line-height:1.6}
.privacy-card>text{display:block;margin-top:15rpx;color:#6d7d8d;font-size:20rpx;line-height:1.7}
.success-card{display:flex;align-items:center;flex-direction:column;gap:12rpx;border-color:#a8d8bc;background:linear-gradient(145deg,#ecfaf2,#fff);text-align:center;color:#3a6a52}
.success-mark{display:grid;width:88rpx;height:88rpx;place-items:center;border-radius:50%;background:#22845b;color:#fff;font-size:50rpx;font-weight:900}
.success-title{font-size:32rpx;font-weight:900;color:#237052}
.meeting-grid>text{min-width:0;overflow-wrap:anywhere}
.section-head>button,.companion-head button{display:flex;align-items:center;justify-content:center;flex-shrink:0;min-width:88rpx;height:64rpx;min-height:34px;margin:0;padding:0 18rpx;border-radius:12rpx;background:#eef5fa;color:#1d628e;font-size:24rpx;line-height:1.2}
.checkin-field{display:flex;min-width:0;flex-direction:column;gap:12rpx;margin-top:24rpx}
.checkin-field>text{display:block;margin:0;color:#536375;font-size:24rpx;line-height:1.5;font-weight:600}
.checkin-field input,.checkin-field textarea{box-sizing:border-box;display:block;width:100%;min-width:0;border:1rpx solid #d5e0e7;border-radius:14rpx;background:#f9fbfc;color:#172033;font-size:28rpx}
.checkin-field input{height:88rpx;min-height:44px;padding:0 20rpx;line-height:normal}
.checkin-field textarea{height:180rpx;min-height:90px;padding:18rpx 20rpx;line-height:1.6}
.checkin-placeholder{color:#8a96a5;font-size:26rpx}
.field-hint{display:block;margin-top:16rpx;color:#7b8999;font-size:22rpx;line-height:1.6}
.companion-card .checkin-field{margin-top:18rpx}
.companion-head{display:flex;align-items:center;justify-content:space-between;gap:16rpx;font-size:24rpx;font-weight:700}
.companion-head button{color:#ad443e;background:#fff3f2}
