...
type SectionMode = 'applications' | 'ledger';
const PAGE_SIZE = 20;
const projectStore = useProjectStore();
const authStore = useAuthStore();
const mode = ref<SectionMode>('applications');
...
const canViewLedger = computed(() => projectId.value > 0
  && authStore.hasProjectPermission(projectId.value, 'seal.view', 'seal.manage', 'seal.export'));
const sectionOptions = computed(() => [
  { key: 'applications' as const, label: '用印申请', description: '发起与审批' },
  ...(canViewLedger.value ? [{ key: 'ledger' as const, label: '用印台账', description: '已审批记录' }] : [])
]);
...
onLoad((options) => {
  projectId.value = getQueryNumber(options?.projectId, 0);
  if (String(options?.mode || '').toLowerCase() === 'ledger') mode.value = 'ledger';
});
onShow(async () => {
  ...
  if (mode.value === 'ledger' && !canViewLedger.value) mode.value = 'applications';
  await refresh();
});
...
async function refresh() {
  ...
  try {
    pageNo.value = 1;
    const result = await getSealApplications({ projectId: projectId.value || undefined,
      scope: mode.value === 'ledger' ? 'ALL' : scope.value,
      status: mode.value === 'ledger' ? 'APPROVED' : status.value,
      keyword: appliedKeyword.value, pageNo: 1, pageSize: PAGE_SIZE });
    applications.value = result.records || [];
    total.value = Number(result.total || 0);
  } catch (error) {
    applications.value = [];
    total.value = 0;
    errorMessage.value = error instanceof Error ? error.message : `${mode.value === 'ledger' ? '用印台账' : '用印申请'}加载失败`;
...
async function loadMore() {
  ...
  try {
    ...
    const result = await getSealApplications({ projectId: projectId.value || undefined,
      scope: mode.value === 'ledger' ? 'ALL' : scope.value,
      status: mode.value === 'ledger' ? 'APPROVED' : status.value,
      keyword: appliedKeyword.value, pageNo: next, pageSize: PAGE_SIZE });
...
async function changeMode(value: SectionMode) {
  if (value === 'ledger' && !canViewLedger.value) return;
...
<template>
  <view class="list-page">
    <AppNavBar title="用印管理" @back="goBack" />
    <view class="section-tabs" role="tablist" aria-label="用印管理功能"><button v-for="item in sectionOptions" :key="item.key" :class="{ active: mode === item.key }" @tap="changeMode(item.key)"><text>{{ item.label }}</text><text>{{ item.description }}</text></button></view>
    <view v-if="mode === 'applications'" class="scope-tabs"><button v-for="item in scopeOptions" :key="item.key" :class="{ active: scope === item.key }" @tap="changeScope(item.key)">{{ item.label }}</button></view>
    <view class="toolbar">
      ...
      <view v-if="mode === 'applications'" class="actions"><button @tap="directApply">选择印章</button><button class="scan" @tap="scanSeal">扫码发起</button></view>
    </view>
    ...
    <scroll-view v-if="mode === 'applications'" class="status-scroll" scroll-x><view class="status-row"><button v-for="item in statusOptions" :key="item.value" :class="{ active: status === item.value }" @tap="changeStatus(item.value)">{{ item.label }}</button></view></scroll-view>
    <view v-else class="ledger-note">展示当前项目已批准的用印记录；Excel 台账请在 Web 端导出。</view>
    ...
    <scroll-view class="application-scroll" scroll-y enable-flex lower-threshold="120" refresher-enabled :refresher-triggered="loading" @refresherrefresh="refresh" @scrolltolower="loadMore">
      <view class="list-content">
        ...
        <button v-for="item in applications" :key="item.id" class="application-card" @tap="navigateTo(`/pages/seal/detail?id=${item.id}`)">
          ...
          <view class="meta"><text>{{ item.projectName }}</text><text>{{ item.applicantName || '当前申请人' }}</text><text>{{ ((mode === 'ledger' ? item.approvalTime : item.submitTime) || item.createTime || '').replace('T',' ').slice(0,16) }}</text></view>
          <view class="file-summary"><text>用印文件 {{ item.items?.length || 0 }} 项</text><text>›</text></view>
        </button>
        <view v-if="loading && !applications.length" class="state-card">正在加载{{ mode === 'ledger' ? '用印台账' : '用印申请' }}…</view>
        <view v-else-if="!applications.length && !errorMessage" class="empty-card"><text>印</text><text>{{ mode === 'ledger' ? '暂无已批准用印记录' : scope === 'PENDING_FOR_ME' ? '暂无待审批申请' : scope === 'CC_TO_ME' ? '暂无抄送申请' : '暂无用印申请' }}</text><text>{{ mode === 'ledger' ? '已批准申请会自动进入台账' : '可选择印章或扫描现场二维码发起' }}</text></view>
...
<style scoped>
.list-page { display: flex; height: 100vh; overflow: hidden; flex-direction: column; background: #f4f6f7; color: #223247; }.section-tabs { display: grid; grid-template-columns: repeat(2,minmax(0,1fr)); gap: 10rpx; padding: 10rpx 24rpx 0; background: #fff; border-bottom: 1rpx solid #e2e7eb; }.section-tabs button { min-height: 72rpx; padding: 8rpx 12rpx 10rpx; border-bottom: 5rpx solid transparent; color: #72808d; text-align: left; }.section-tabs button.active { border-bottom-color: #315f86; color: #315f86; }.section-tabs text { display: block; }.section-tabs text:first-child { font-size: 23rpx; font-weight: 820; }.section-tabs text:last-child { margin-top: 3rpx; color: #98a2ad; font-size: 17rpx; }
.scope-tabs { display: grid; grid-template-columns: repeat(3,minmax(0,1fr)); gap: 8rpx; margin: 8rpx 24rpx 0; padding: 7rpx; border-radius: 16rpx; background: #e7edf1; }.scope-tabs button { min-height: 58rpx; border-radius: 12rpx; color: #6e7d8c; font-size: 21rpx; font-weight: 700; }.scope-tabs button.active { background: #fff; color: #315f86; box-shadow: 0 4rpx 14rpx rgba(49,95,134,.09); }
...
.status-scroll { width: 100%; flex-shrink: 0; white-space: nowrap; }.status-row { display: inline-flex; gap: 9rpx; padding: 0 24rpx 12rpx; }.status-row button { min-height: 50rpx; padding: 0 18rpx; border-radius: 999rpx; background: #e8edf1; color: #74818e; font-size: 19rpx; }.status-row button.active { background: #dce9f0; color: #315f86; font-weight: 750; }.ledger-note { flex-shrink: 0; margin: 0 24rpx 12rpx; padding: 13rpx 16rpx; border-radius: 12rpx; background: #e9eff5; color: #607589; font-size: 18rpx; line-height: 1.45; }.application-scroll { min-height: 0; flex: 1; }.list-content { display: flex; flex-direction: column; gap: 14rpx; padding: 4rpx 24rpx 34rpx; }
