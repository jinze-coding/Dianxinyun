...
新字段与审批结果原子保存，旧记录保持按需上传。原本地 `dianxinyun` 已在完整备份和隔离副本验证后双跑
`20260911_seal_optional_attachments.sql`，仍为 106 表，原服务已重启。生产应先执行增量再启动新后端，
不得重建已有数据库。接口、迁移及验证入口见 [用印附件专题](../docs/用印附件与盖章件要求.md)。

## 2026-09-11 当前本地与会议资料

当前原 `dianxinyun` 开发库为 106 表，禁止重新运行 init.sql。会议资料迁移依赖 20260902、20260910 两项迁移，
本地已备份、预检并执行 20260911_meeting_materials.sql 两遍。新增预览转换器需随服务部署；macOS 使用受限
本地 LibreOffice/FFmpeg，Linux 使用禁网容器。配置、接口和验证见 [会议资料专题](../docs/会议完整详情与资料留档.md)。
