...
- 场内管理：Web 创建单次外访邀请、查看/纠错/作废并按人员导出；访客通过专属小程序码免注册填报，并可在微信静默身份下复用当前项目的常用人员与车辆资料。
...
backend/
  src/main/java/com/example/siteplatform/
    ...
    siteaccess/    场内外访邀请、常用访客资料、实名人员、加密审计和导出
...
- 场内外访邀请删除必须按人员实名信息、场内业务审计、邀请主记录的顺序在同一事务内完成；不级联删除独立的常用资料，`sys_operation_log` 不级联，并新增本次管理员删除日志。项目包含邀请、常用资料或其审计时均不得直接物理删除。
...
- 场内邀请令牌只能通过请求体传输，不得进入 URL、SQL 参数日志、异常信息或普通操作日志。手机号、身份证号和令牌明文必须使用独立 `VISITOR_DATA_ENCRYPTION_KEY` 做 AES-GCM 加密；只保存令牌摘要用于检索。公开解析与提交必须先按受信 IP 限流，提交事务通过行锁保证只能成功一次。
...
- 当前有数据的本地 `dianxinyun` 已完成统一注册、RBAC、微信快捷登录、项目多角色、
  跨端业务模块、质量并发、巡检整改、场内外访、常用访客资料和用印一期迁移，当前为 64 表。只有从迁移前
  39 表旧副本升级时，
