...
backend/
  src/main/java/com/example/siteplatform/
    ...
    siteaccess/    场内外访邀请、常用访客资料、人员明细、加密审计和导出
...
- Web 页签菜单固定为资料库/用印申请/用印台账/回收站、电箱台账/巡检记录、周检记录/整改闭环/质量资料；页签细分不得将小程序拆成另一套业务授权。
...
- 场内外访邀请删除必须按人员明细、场内业务审计、邀请主记录的顺序在同一事务内完成；不级联删除独立的常用资料，`sys_operation_log` 不级联，并新增本次管理员删除日志。项目包含邀请、常用资料或其审计时均不得直接物理删除。
...
- 场内邀请令牌只能通过请求体传输，不得进入 URL、SQL 参数日志、异常信息或普通操作日志。访客手机号、历史身份证号和令牌明文必须使用独立 `VISITOR_DATA_ENCRYPTION_KEY` 做 AES-GCM 加密；只保存令牌摘要用于检索。同行人员单位、姓名、手机号均为选填，完全空白同行项不得写库或计入人数，填写手机号时仍必须校验格式。公开解析与提交必须先按受信 IP 限流，提交事务通过行锁保证只能成功一次。
...
- 当前有数据的本地 `dianxinyun` 已完成统一注册、RBAC、微信快捷登录、项目多角色、
  跨端业务模块、质量并发、巡检整改、场内外访、常用访客资料、门卫登记和用印一期迁移，当前为
  68 表。只有从迁移前
  39 表旧副本升级时，
...
增加可空周检归属与条目顺序；历史独立问题不回填虚构批次。该脚本当前只随代码准备和隔离验证，
