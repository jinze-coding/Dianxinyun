...
- 场内管理：Web 在同一预约列表创建 `SINGLE` 单次外访或 `MEETING` 共享会议邀请，并可查看、纠错、作废和按人员导出。单次预约保持一次提交锁定整条邀请；会议码在截止前允许不同微信身份分别登记，每个身份只查看自己的登记组凭证，单组作废后可重新登记。访客免注册，可在微信静默身份下复用当前项目的常用人员与车辆资料；截止或整场作废后不再返回凭证和导航。
...
- 资料、巡检、质量必须通过一个模块开关统一控制两端入口。场内管理的模块开关只控制 Web 内部管理；小程序 `pages/public/visitor-invite` 和 `pages/public/meeting-invite` 是持邀请令牌的免登录公开页，不进入内部菜单。关闭模块时保留细分操作权限配置，但对应内部入口和后端项目鉴权必须拒绝该模块。
...
- 场内外访邀请删除必须在同一事务中先删除会议登记人员、会议审计、会议登记组，再删除单次人员明细、场内业务审计和邀请主记录；不级联删除独立的常用资料，`sys_operation_log` 不级联，并新增本次管理员删除日志。项目包含邀请、会议登记、常用资料或其审计时均不得直接物理删除。
