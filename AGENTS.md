...
- 场内管理：Web 创建单次外访邀请、查看/纠错/作废并按人员导出；访客通过专属小程序码免登录一次性填报。
...
- 场内外访邀请删除必须按人员实名信息、场内业务审计、邀请主记录的顺序在同一事务内完成；`sys_operation_log` 不级联，并新增本次管理员删除日志。项目仍不得在包含外访记录时直接物理删除，管理员应先逐条确认清理外访邀请。
...
- 当前有数据的本地 `dianxinyun` 已完成统一注册、RBAC、微信快捷登录、项目多角色、
  跨端业务模块、质量并发、巡检整改、场内外访和用印一期迁移，当前为 61 表。只有从迁移前
  39 表旧副本升级时，
