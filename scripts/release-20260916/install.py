#!/usr/bin/env python3
"""2026-09-16 code-only update of the verified 2026-09-15 installation."""
import fcntl
import grp
import hashlib
import json
...
OLD_JAR_SHA = '0de87d1833edbaa41040f836a69939aa1aca6bf447863f5aeb4dc3da2a99a921'
OLD_HTML_SHA = 'b59f46a4d03b4518522fc34c07cb1f16ab4e82d89724b258e2d21fbb626a9558'
...
def main():
    ...
    with open('/run/site-platform-release.lock', 'a') as release_lock, open('/run/site-platform-watchdog.lock', 'a') as watchdog_lock:
        ...
        require(release_id.startswith('20260916-') and release_id.replace('-', '').isdigit(), '版本标识不正确')
        ...
        require(sha(old_jar) == OLD_JAR_SHA and sha(old_web / 'index.html') == OLD_HTML_SHA,
                '当前线上代码与 9 月 15 日正式基线不符，尚未停服，请提供当前版本信息')
