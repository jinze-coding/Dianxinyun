...
if [ "$mini_state" = released ]; then
  require_confirmation MINI_NEW_VERSION_ROLLBACK_OR_OLD_BACKEND_COMPATIBILITY_APPROVED "$mini_confirmation"
fi
...
if [ "$DRY_RUN" = 1 ]; then
  ...
  [ "$mini_state" = released ] && log 'DRY-RUN：已声明小程序 0.1.10 发布，正式执行时必须另行确认客户端回退或旧后端兼容性'
  exit 0
fi
...
log "同点回滚完成：旧数据库 98 表/25 标记、配对 uploads、旧 JAR/Web/env/Nginx/systemd 已恢复"
