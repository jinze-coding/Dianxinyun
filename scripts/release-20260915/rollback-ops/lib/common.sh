...
assert_exact_legacy_baseline() {
  ...
  compare_exact_file "$EXPECTED_TABLES_FILE" "$actual_tables" '98 张正式旧表清单'
  compare_exact_file "$EXPECTED_MARKERS_FILE" "$actual_markers" '25 个正式旧迁移标记'
  [ "$(wc -l < "$actual_tables" | tr -d ' ')" = 98 ] || die '旧表数量不是 98'
  [ "$(wc -l < "$actual_markers" | tr -d ' ')" = 25 ] || die '旧迁移标记数量不是 25'
  rm -rf -- "$temp_dir"
}
