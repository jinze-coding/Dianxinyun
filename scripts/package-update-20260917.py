#!/usr/bin/env python3
"""Package the reviewed 2026-09-17 backend/Web and additive database update from frozen source."""
import argparse
import hashlib
...
def main():
    ...
    with zipfile.ZipFile(args.jar) as jar:
        ...
        require(all(name in jar.namelist() for name in ('BOOT-INF/classes/com/example/siteplatform/project/service/ProjectAccessBatchService.class', 'BOOT-INF/classes/com/example/siteplatform/system/correction/CorrectionService.class', 'BOOT-INF/classes/com/example/siteplatform/quality/dto/QualityWeeklyReturnRequest.class')), 'missing new business code')
    require(args.release_id.startswith('20260917-') and args.release_id.replace('-', '').isdigit(), 'invalid release ID')
    ...
    with tarfile.open(args.source) as source, tarfile.open(args.baseline_source) as baseline, tarfile.open(args.web) as web, tempfile.TemporaryDirectory(prefix='dxy-code-package-') as temp:
        ...
        allowed = {'backend/src/main/resources/sql/empty-database-baseline.sql.template', 'backend/src/main/resources/sql/migrations/20260916_administrator_data_correction.sql'}
        require(not [p for p in changes if p.startswith(protected) and p not in allowed], 'unexpected migration, configuration, dependency or converter change')
        ...
        for name in ('install.py', 'run.sh'):
            write(name, new_files['scripts/release-20260917/' + name])
        write('code_install.py', new_files['scripts/release-20260916/install.py'])
        write('database.py', new_files['scripts/release-20260917/database.py'])
        write('database/20260916_administrator_data_correction.sql', new_files['backend/src/main/resources/sql/migrations/20260916_administrator_data_correction.sql'])
        write('README-更新说明.md', new_files['docs/正式更新准备-20260917.md'])
        ...
        write('RELEASE.json', json.dumps({
            ...
            'worktree': source_manifest['WORKTREE_STATE'], 'databaseChanges': True,
            'databaseMigrations': ['20260916_administrator_data_correction.sql'],
            ...
            'validation': {'backendCorrectionSyntheticIntegrationTests': 30, 'backendWeeklyReturnIntegrationTests': 5,
                           'webUnitTests': 196,
        ...
        print(json.dumps({'archive': str(args.output), 'bytes': args.output.stat().st_size, 'sha256': outer_sha,
                          'sourceManifestSha256': source_hash, 'databaseChanges': True}, indent=2))
