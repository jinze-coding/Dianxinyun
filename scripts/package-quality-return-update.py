#!/usr/bin/env python3
"""Package the reviewed 2026-09-17 weekly-return backend/Web patch from frozen source."""
import argparse
import hashlib
...
def main():
    ...
    require(sha(baseline_raw) == '5065e3c7a2431a0bd88a5dcdb4ce3c2eea062d560df0fc9de5301039b77f967a', 'wrong production baseline')
    ...
    with tarfile.open(args.source) as source, tarfile.open(args.baseline_source) as baseline, tarfile.open(args.web) as web, tempfile.TemporaryDirectory(prefix='dxy-code-package-') as temp:
        ...
        require(set(p for p in changes if p.startswith(('backend/src/main/', 'frontend/src/'))) <= {'backend/src/main/java/com/example/siteplatform/quality/service/QualityWeeklyInspectionService.java', 'frontend/src/pages/QualityManagement/WeeklyInspectionPanel.jsx'}, 'unexpected runtime source change outside weekly return patch')
        ...
        for name in ('install.py', 'run.sh'):
            write(name, new_files['scripts/release-20260917-quality-return/' + name])
        write('README-更新说明.md', new_files['docs/正式更新准备-20260917-周检回退.md'])
        ...
        write('RELEASE.json', json.dumps({
            ...
            'validation': {'backendRelatedUnitTests': 46, 'backendSyntheticIntegrationTests': 9,
